...
import { Bell, CheckCircle2, Edit2, X, Clock, AlertTriangle } from "lucide-react";
...
/**
 * FollowupPanel — Single Active Followup UX
 * Props: client, onUpdate(updatedFields), onFollowupDone()
 */
export default function FollowupPanel({ client, onUpdate, onFollowupDone }) {
  ...
  // ── A: No active followup → show creation form ──────────────────────────
  const handleSchedule = async (iso, note) => {
    ...
    try {
      ...
      // If no first contact yet, prompt
  ...
  return (
    <div className="space-y-3">
      ...
      {/* ── After "done" → prompt next followup ── */}
      <AnimatePresence>
        {showNextPrompt && (
          <FollowupPrompt
            leadId={client.id}
      ...
      {/* ── After scheduling without first contact ── */}
      <AnimatePresence>
