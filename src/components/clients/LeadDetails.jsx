...
function FollowupPanel({ client, onUpdate }) {
  ...
  const save = async (nextDate, nextNote) => {
    ...
    setIsSaving(true);
    try {
      // nextDate is a "YYYY-MM-DDTHH:mm" string in Israel local time — convert to UTC (DST-safe)
      const utcIso = israelLocalToUtcIso(nextDate);
...
export default function LeadDetails({ client: initialClient, meetings, onClose, onEdit, onCreateMeeting, onRefresh }) {
  ...
  // contactCycleOpen: true = contacted (show tag + followup done btn), false = show "סמן נוצר קשר"
  // Opens on markFirstContact, closes on markFollowupDone
