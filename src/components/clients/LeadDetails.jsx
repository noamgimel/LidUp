...
function FollowupPanel({ client, onUpdate }) {
  ...
  const save = async (nextDate, nextNote) => {
    ...
    await base44.entities.Client.update(client.id, {
      next_followup_at: nextDate ? new Date(nextDate).toISOString() : null,
    ...
    await base44.entities.LeadActivity.create({
      ...
      content: `פולואפ נקבע ל-${nextDate ? formatIsraeliDate(nextDate) : ""}${nextNote ? ` — ${nextNote}` : ""}`,
      created_by_email: user?.email || ""
    });
    setIsSaving(false);
    onUpdate?.();
...
export default function LeadDetails({ client: initialClient, meetings, onClose, onEdit, onCreateMeeting, onRefresh }) {
  ...
  const handleFirstResponse = async () => {
    ...
    await base44.entities.Client.update(client.id, {
      ...
      priority: client.priority === "overdue" ? "warm" : client.priority
    });
    await base44.entities.LeadActivity.create({
      lead_id: client.id,
